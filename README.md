...
- LetsEncrypt certificates are only generally suitable for use on single-node systems. LetsEncrypt cannot be used to terminate SSL on load balanced hosts which individually generate their own certs. At least, not with this role. Fortunately most cloud providers support their own certificates for use in TLS-terminating load balancers, in which case you would probably not use this role for TLS termination at all.
...
nginx_listeners:
  ...
  - port: 443
    ...
    ## OCSP stapling is now deprecated in favor of Certificate Revocation lists. If the server uses an OCSP "must staple" certificate, then also provide the following. These shouldn't be needed in most cases:
    # ssl_ocsp_must_staple: true
